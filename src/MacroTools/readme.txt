...
New in MacroTools v1.0
********************

Repeatable playback
--------------------
To repeat playback, check the "Repeat Playback" checkbox and type in the no. of times you want to repeat playback in the "No. of Repetitions" textbox. Remember, it's the number of repetitions, so if you type 1 in there, MacroTools will play back the file twice. 
...
anything useful with it.
  ...
  MacroTools v1.0 comes with the ability to adjust playback rate.  To adjust playback speed, Click the Options button in the main MacroTools window. It
...
Using MacroTools is very simple, in fact, you can't go wrong anywhere.
...
*********************
1. Ctrl+Alt+Del, Ctrl+Esc and Ctrl+Break keys break the recording or playing back process, by evicting the hook. That means you can't record or playback those keystrokes. But if MacroTools ever hangs, you can easily recover control by using any of those keystrokes.

2. The Windows key doesn't work properly with MacroTools. Trouble is, it gets recorded as  some other keystroke, and playing it back can give weird results. So you'd better move your mouse to the Start button than rely on the Windows key.

3. MacroTools is intelligent enough to recognise files it recorded, so you can't simply play any file. But if you ever fiddle with the recorded file yourselves, the application may hang. Of course, you can always use the keystrokes mentioned above to recover. 

4. And remember, MacroTools simply records mouse and keyboard messages. So if you have an application that opens up in different window coordinates everytime you open it, then your mouse actions on that window may not be played back correctly. Notepad on Windows 98 has this behaviour. It opened up on center screen while I was recording, and opened on the left corner of the desktop when I played back. As a result, my mouse clicks on the window didn't play back correctly. However, you wouldn't have any problem with keystrokes, and you can always use Alt+F4 to close any application :)
